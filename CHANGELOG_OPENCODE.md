...
## 16. GitHub Pages 404 Fix — إضافة GitHub Actions Workflow

### المشكلة
- الموقع مرفوع على GitHub Pages لكنه يفتح 404
- السبب: لا يوجد GitHub Actions workflow لرفع مجلد `dist` إلى Pages

### التغيير
- إضافة ملف `.github/workflows/deploy.yml`:
  - `on: push` على `main` + `workflow_dispatch`
  - Build باستخدام `npm ci` و `npm run build`
  - رفع مجلد `dist` كـ `upload-pages-artifact`
  - نشر باستخدام `deploy-pages`
...
| الملف | التغيير |
|---|---|
| `.github/workflows/deploy.yml` | **ملف جديد** — GitHub Actions workflow للنشر التلقائي على Pages |
